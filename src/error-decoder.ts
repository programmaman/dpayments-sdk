...
const ERROR_IFACE = new Interface([
  ...
  'error InvalidPaymentAmount()',
  'error PaymentAlreadyExists(bytes32 id)',
 ...
 * does not match any known DisputablePayment / PaymentFactory contract error.
 *
 * @example
 * ```ts
 * try { await wallet.sendTransaction(tx); }
 * catch (err) {
 *   const r = decodeDPaymentError(err);
 *   if (r?.error) console.error(r.error, r.args);
 * }
 * ```
 */
export function decodeDPaymentError(err: unknown): DecodedRevert | null {
